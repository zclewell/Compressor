...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
	...
	{
		TreeNode * newLeft = removeSmallest(singleQueue,mergeQueue);
		TreeNode * newRight = NULL;
		if(singleQueue.size() + mergeQueue.size() >0)
		{
			newRight = removeSmallest(singleQueue,mergeQueue);
		}
		TreeNode * temp = new TreeNode(newLeft->freq.getFrequency() + newRight->freq.getFrequency());
		temp->left = newLeft;
		temp->right = newRight;
